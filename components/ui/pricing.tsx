  ...
  return (
    <div className="container py-20">
      ...
      <div className="flex justify-center items-center gap-4 mb-10">
        <span
          className={`font-semibold transition-colors ${isMonthly ? "text-white" : "text-gray-500"}`}
        >
          1 tháng
        </span>
        ...
        <span
          className={`font-semibold transition-colors ${!isMonthly ? "text-white" : "text-gray-500"}`}
        >
          1 năm <span className="text-blue-400">(Tiết kiệm 20%)</span>
        </span>
      </div>
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="mt-6 flex items-center justify-center gap-x-2">
                <span className="text-5xl font-bold tracking-tight text-white transition-all duration-500 ease-out">
                  {(isMonthly ? plan.price : plan.yearlyPrice) === "Liên hệ"
                    ? "Liên hệ"
                    : `${(parseFloat(isMonthly ? plan.price : plan.yearlyPrice) * 1000).toLocaleString("vi-VN")}`}
                </span>
                {plan.period &&
                  (isMonthly ? plan.price : plan.yearlyPrice) !== "Liên hệ" && (
